...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   追加先要素のタグ名とクラス名」の内訳を最大10件表示する。効果文データの
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が表示されている要素、見つからなければカード名の要素）そのものの
   末尾に、子として `<br>` と `<span class="czn-effect-text-ja">`（文字色
   のみ指定、薄い黄色）を直接追加する。新しいブロック/兄弟要素を作る方式は
   いずれも実際には幅・高さ0で非表示になり表示に失敗したため、既に画面に
   表示されている要素にテキストを足す形にした。挿入のたびに、box の中に
   既存の追加分（spanとその直前のbr）があれば `querySelectorAll` で全て
   探して削除してから作り直すため、SPA側の再描画で同じboxが複数回処理
   されても積み上がることはない。カード名の要素に追加した場合、次回の
   カード名解析では追加したspan/brを除いた本来の名前テキストだけを読み
   取る（そうしないと自分が追記した日本語効果文までカード名として誤読して
   しまうため）。英語の効果文はどこにあっても消さずそのまま残す。該当level
   の効果文が無ければ何もしない
