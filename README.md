...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   効果文が1件以上挿入できたときは、実際に挿入した「原文 / level / ページ
   上のtop座標 / 表示中か非表示か」の内訳を最大10件表示する。効果文データの
   件数と挿入件数が一致しない（重複挿入やlevel違いへの誤挿入が疑われる）
   ときに、その場でどのカードに何が入ったか、それらが本当にページ上の別々
   の位置にある別要素なのか（＝正しい挙動）を確認できる。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が見つかれば、カード枠（box）の「中」でも box の兄弟要素でもなく、
   box の親要素（グリッドのセルに相当）自体の末尾に、独立したブロックで
   日本語の効果文を追加する（box自身が `position:absolute` でレイアウト
   されている可能性を考慮し、より外側に置く）。`position:static`・
   `display:block`・`width:100%`+`box-sizing:border-box`、
   `white-space:normal`・`word-break:normal`、文字サイズはカード名見出しの
   実測値に合わせ、背景は濃い半透明の黒、文字は白で表示する。挿入のたびに、
   挿入先コンテナの中に既存の効果文ブロックがあれば `querySelector` で
   探して削除してから作り直すため、SPA側の再描画でboxのdata属性だけが
   失われるようなケースでも同じカードに複数のブロックが並ぶことはない。
   英語の効果文がどこにあっても消さずそのまま残す。該当levelの効果文が
   無ければ何もしない
