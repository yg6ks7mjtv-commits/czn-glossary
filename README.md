...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   効果文が1件以上挿入できたときは、実際に挿入した「原文 / level」の
   内訳を最大10件表示する。効果文データの件数と挿入件数が一致しない
   （重複挿入やlevel違いへの誤挿入が疑われる）ときに、その場でどのカード
   に何が入ったかを確認できる。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   重なってしまうため）。`position:static`・`display:block`、幅と文字
   サイズはそれぞれ box・カード名見出しの実測値に合わせ、背景は濃い半透明の
   黒、文字は白で表示する。挿入のたびに、box の直後に既存の効果文ブロックが
   あれば一度削除してから作り直すため、SPA側の再描画でboxのdata属性だけが
