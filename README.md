...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が見つかれば、カード枠内の効果文要素（コスト数字・カード名・種別
   表示・「Show Effects」を除いた最長のテキスト要素、＝既に確実に画面に
   表示されている英語の効果文の要素）そのものを書き換える。新しい要素を
   挿入する方式は幅・高さが0に潰れて表示に失敗したため採用していない。
   背景を白、文字色を黒、角丸を付け、中身を日本語の効果文に差し替え、
   元の英語は消さず `title` 属性に退避する（ホバーで見られるが画面表示は
   日本語のみになる）。書き換えた要素には `data-czn-done="1"` を付けて
