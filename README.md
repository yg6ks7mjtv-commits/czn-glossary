...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が見つかれば、カード枠（box）自体の末尾に、独立したブロックとして
   日本語の効果文を追加する。カード画像に重ねて表示するのではなく、
   カードの下に地続きのブロックとして表示する（背景は濃い半透明の黒、
   文字は白、文字サイズはカード名見出しに合わせる）。英語の効果文がどこに
   あっても消さずそのまま残す。該当levelの効果文が無ければ何もしない
