...
（英語の効果文と「Show Effects」リンクが入っている領域）に日本語の効果文を
追記する機能を持つ。**Chrome ウェブストアには公開しない。**
デベロッパーモードでフォルダを読み込んで使うローカル専用の拡張。
...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件を検出 /
   効果文◯件を挿入」というトーストが出る。何も処理できなかった場合は
   「CZN: 対象が見つかりません」と出る（マーカー検出やセレクタが実際の
   ページに合っていない可能性が高い）。効果文の挿入が0件のときは、検出できた
   カード名ごとに「日本語化できたか」「効果文が見つかったか」も表示される
...
1. カード名を原文のまま収集する（用語置換より前に必ず終わらせる。順序を
   ...
   失敗するため）
2. `glossary.json` の英語カード名と完全一致し、かつ末尾がコロンでない
   （＝「Starting Cards:」のようなグループ見出しではない）ものだけを
   カード名候補として採用する
3. 「Show Effects」を含む要素から親を1階層ずつたどり、上記の条件を満たす
   カード名候補が現れた時点で確定する（＝カード名とShow Effectsの両方を
   含む最小の共通祖先を「カード枠」とみなす。それより外は探索しない）
4. `effects-ja.json` をそのカード名（日本語）で検索し、見つかれば英語の
   説明文を消さずにその下へ追記する。効果文が無いカードは何もしない
5. 最後に、カード名以外のテキストの用語置換をブックマークレットと同じ
   アルゴリズム・同じ辞書（`glossary.json`）で行う

### 実装上の注意（重要）

**`www.prydwen.gg` への自動アクセスが403で拒否される環境で書いたため、
実際のカード要素のHTML構造の細部（クラス名など）は見ないまま書いている。**
カードの説明欄自体は空ではなく、Prydwenの英語の効果説明と「Show Effects」
リンクが入っていることは分かっているので、それを手がかりに検出する設計にした。

- 既定の検出方式（`extension/selectors.js` の `useMarkerStrategy: true`）は、
  固定の階層数や見出しタグの総当たりではなく、**glossary名との完全一致**を
  カード名の判定基準にしている。これにより「Starting Cards:」等のグループ
  見出しを誤ってカード名として拾う事故を防いでいる
- マーカーの文字列・たどる最大階層数（`maxAncestorClimb`）はすべて
...
- 正確なCSSセレクタが判明したら、`selectors.js` の `useMarkerStrategy` を
  ...
  マーカー方式ではなくセレクタ直指定の経路に切り替えられる
- `content.js` 冒頭の `CZN_DEBUG` を `true` にすると、マッチ状況を
  console に出力する

一発では動かない前提で作ってあるので、画面右下のトースト表示（カード枠の
検出件数・効果文の挿入件数・0件時は検出名ごとの診断）と `CZN_DEBUG` を
見ながら `selectors.js` を育てていくこと。
