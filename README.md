...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   候補（コスト数字・カード名・種別表示・ローマ数字バッジ、カード名と
   同一テキストの重複要素を除いた葉要素）は実測幅の広い順に並べる。
   ...
   **候補の採用可否は事前の条件判定ではなく、実際に書き換えてみた結果を
   ...
   (b) 幅が探索範囲自体の幅の60%未満、のいずれかに該当すれば失敗と判定して
   元のテキスト・スタイルに戻し、次の候補を試す。候補が尽きれば英語のまま
   残す。
