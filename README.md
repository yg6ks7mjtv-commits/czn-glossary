...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   書き換える。候補（コスト数字・カード名・種別表示・ローマ数字バッジを
   除いた葉要素）は、`[Linked]`/`[Unique]` のような角括弧タグを含むもの・
   「Show Effects」と同じ親要素を共有するもの・カード名要素より下（DOM順で
   後）にあるものを優先するスコアリングで選ぶ（単純な最長文字列だけを基準に
   すると、カード名の重複要素——幅固定・省略表示のCSSを持つ別要素——を
   誤って選んでしまうことがあったため）。カード名と同一テキストの重複要素も
   候補から除外する。新しい要素を挿入する方式は幅・高さが0に潰れて表示に
   失敗したため採用していない。背景を白、文字色を黒、角丸を付け、
