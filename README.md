...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   「原文 / level / タグ名とクラス名 / 選ばれた要素の幅(カード枠の幅) /
   書き換え前のテキスト冒頭30文字」を最大10件表示する。カード名の要素などを
   誤って書き換えていないか、幅の狭い帯になっていないか、その場で確認できる。
...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   重複要素も候補から除外する。**実測幅がカード枠の60%未満の要素も候補から
   除外する**（英語の効果文は横幅いっぱいに広がっているはずなので、幅の
   狭い要素——縦書きのような細い帯になる原因——を弾く）。それでも候補が
   見つからなければ、その範囲内で最も文字数の多い要素（カード名等を除く。
   幅の条件は維持）を条件を課さず無条件で採用する最終フォールバックがある。
   新しい要素を挿入する方式は幅・高さが0に潰れて表示に失敗したため採用して
   いない。背景を白、文字色を黒、角丸を付け、`white-space:normal`・
   `word-break:normal`・`writing-mode:horizontal-tb`・`width:auto`・
   `min-width:0` を強制してカード名側の省略表示CSSや縦書き指定の影響を
   打ち消し、中身を日本語の効果文に差し替え、元の英語は消さず `title`
   属性に退避する（ホバーで見られるが画面表示は日本語のみになる）。
   日本語の効果文を
