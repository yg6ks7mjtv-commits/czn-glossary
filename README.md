...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   「原文 / level / タグ名とクラス名 / 候補◯件中◯番目を採用 / 各候補の
   書き換え後の幅と高さ（例: `1:120x300(失敗)、2:340x40(採用)`）」を
   最大10件表示する。カード名の要素などを誤って書き換えていないか、
   なぜその候補が採用されたのか、その場で確認できる。
   ...
   確認できる。試した候補があれば各候補の書き換え後の幅と高さも表示する。
   書き換え先の要素が1つも見つからなかった場合は、名前照合用のカード枠
   （box）自体のタグ名・クラス名・中のテキスト要素数・`textContent` 冒頭
   50文字に加えて、実際に探索範囲として採用した scope（box から内容の
   増分を基準に広げた後の要素。boxとは別に表示する）のタグ名・クラス名・
   登った階層数・中のテキスト要素数・`textContent` 冒頭50文字も併せて
   表示する（box が小さすぎるのか、scopeへの拡大が機能していないのか、
   除外条件が厳しすぎるのかを切り分けるため）。さらに、scope内で走査した
   葉要素そのものを最大8件、タグ名・クラス名・幅と高さ・`textContent`
   先頭20文字・除外理由（カード名／処理済み／その他（空）／候補として
   採用）とともに表示する（除外条件で候補が0件になった場合に、何が・なぜ
   除外された/されなかったかをその場で確認できるようにするため）。
...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   別に扱う**: box から親を1階層ずつたどり、その要素の `textContent` が
   box の `textContent` より50文字以上多くなった時点で、そこを探索範囲
   として採用する（＝効果文相当のまとまった量のテキストが新たに含まれる
   ようになったとみなす。box がカード名＋種別表示だけのヘッダー部分で、
   効果文が全く別の場所にあるケースに対応するため。固定の階層数だけで
   判断すると届かないことがあった）。最大 `maxEffectSearchClimb`
   （`selectors.js`、既定10）階層まで許可。ただし2つ目の種別表示を含む
   階層に達したら、その1つ手前で打ち切る（3.の歯止め(a)と同じ考え方）。
   候補は探索範囲内の葉要素のうち、カード名要素（c.nameEl）・既に
   書き換え済み（`data-czn-done="1"`）・textContentが空の3種類だけを
   除いた残り全てで、実測幅の広い順に並べる。種別表示・コスト数字・
   ローマ数字バッジ・「Show Effects」等はあえて候補から除外しない
   （テキスト内容で事前に除外すると、効果文の要素自体まで巻き込んで
   候補が0件になることがあったため）。

   **候補の採用可否は事前の条件判定ではなく、実際に書き換えてみた結果を
   実測して判断する**（角括弧タグや幅の事前フィルタだけでは、縦書きになる
   誤選択を防ぎきれなかったため）。幅の広い候補から順に、日本語テキストと
   スタイル（背景白・文字黒・角丸、`white-space:normal`・
   `word-break:normal`・`writing-mode:horizontal-tb`・`width:auto`・
   `min-width:0` を強制）を実際に適用してから `getBoundingClientRect()` で
   幅・高さを測定し、(a) 高さが幅より大きい（縦書きになっている）、また
   (b) 幅が0（非表示要素）、のいずれかに該当すれば失敗と判定して元の
   テキスト・スタイルに戻し、次の候補を試す。カード枠に対する相対幅
   （60%等）による事前の足切りはしない。候補が尽きれば英語のまま残す。
   採用できた場合、中身は日本語の効果文に差し替わり、元の英語は消さず
   `title` 属性に退避する（ホバーで見られるが画面表示は日本語のみになる）。
   日本語の効果文を
