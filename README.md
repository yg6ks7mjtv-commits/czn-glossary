...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   「原文 / level / タグ名とクラス名 / 書き換え前のテキスト先頭30文字」を
   ...
   （box から内容の増分を基準に広げた後の要素。boxとは別に表示する）の
...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   **このサイト上では候補要素の `getBoundingClientRect()` が、画面上に
   表示されているにもかかわらず常に0x0を返すことが判明しており、幅・高さの
   実測による選択や書き換え後の検証・ロールバックは機能しない**（以前の
   バージョンはこれに依存していたが動作しなかった）。そのためサイズに
   基づく判定は全廃し、探索範囲内の葉要素のうちカード名要素（c.nameEl）・
   既に書き換え済み（`data-czn-done="1"`）・textContentが10文字未満の
   3種類だけを除いた残りの中から、`textContent` の文字数が最も多いものを
   無条件で1つ採用する（効果文は他のUI文言より明らかに長いという前提。
   種別表示・コスト数字・ローマ数字バッジ・「Show Effects」等も候補から
   除外しない）。採用した要素には日本語テキストとスタイル（背景白・文字黒・
   角丸、`white-space:normal`・`word-break:normal`・
   `writing-mode:horizontal-tb`・`width:auto`・`min-width:0` を強制）を
   適用する。書き換え後の再測定・ロールバックは行わない。中身は日本語の
   効果文に差し替わり、元の英語は消さず `title` 属性に退避する（ホバーで
   見られるが画面表示は日本語のみになる）。
   日本語の効果文を
