...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   疑われる）ときや、挿入したブロックが画面に出ないときに、中身が入って
   いるか（文字数）とDOM構造（outerHTML）をその場で確認できる。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   絶対に失敗しない。挿入する `div` はインラインスタイルを一切持たない
   素の要素にしてある（box内に置いても幅・高さが0のまま画面に出ない問題を
   切り分け中のため。原因判明後にスタイルを戻す）。挿入のたびに、box の中に
