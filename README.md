...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   効果文が1件以上挿入できたときは、実際に挿入した「原文 / level / 幅×高さ /
   表示中か非表示か」の内訳を最大10件表示する。効果文データの件数と挿入
   件数が一致しない（重複挿入やlevel違いへの誤挿入が疑われる）ときや、
   挿入したブロックが非表示になってしまうときに、その場で確認できる。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が見つかれば、box の「中」、英語の効果文がもともと表示されている
   場所の直後に、日本語の効果文をプレーンなブロックとして追加する
   （box の外に出す方式は、新しい兄弟要素がgrid/flexのレイアウトに巻き込ま
   れて幅・高さ0になり非表示になってしまったため採用していない）。
   挿入位置は「Show Effects」リンクがあればその直後（リンクの内側に紛れ
   込まないよう `closest('a, button')` でクリック領域まで遡ってから
   挿入する）、無ければ効果文の最後のテキスト要素の直後。
   `position` や `width` は指定せず周囲の要素にそのまま従わせ、背景は
   付けず、文字色を白、上に細い区切り線を1本入れ、文字サイズは英語の効果文
   と同じ実測値に合わせる。挿入のたびに、box の中に既存の効果文ブロックが
   あれば `querySelectorAll` で全て探して削除してから作り直すため、SPA側の
   再描画でboxのdata属性だけが失われるようなケースでも複数のブロックが
   並ぶことはない。英語の効果文はどこにあっても消さずそのまま残す。
   該当levelの効果文が無ければ何もしない
