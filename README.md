...
- `scripts/fetch_aosns_effects.py` — nightmare.aosns.comのキャラクターページを
  直接HTTP取得し、「カード評価」セクション内のヒラメキ別効果文（構造化された
  `<li>` 形式のページのみ）をHTML構造から解析して抽出する補助ツール
  （`effects-ja.json` は自動更新しない。標準出力にJSONを出すだけ）。全キャラの
  ページがこの構造を持つわけではなく、古いページや文章が段落形式のページ
  では何も抽出できない（そのキャラはスキップされる。詳細はスクリプト冒頭の
  コメント参照）
