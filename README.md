...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   該当levelの効果文が見つかれば、カード枠内の効果文要素（コスト数字・
   カード名・種別表示・「Show Effects」を除いた最長のテキスト要素）の
   直後に、英語を消さず日本語の効果文を追記する。無ければ何もしない
