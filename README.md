...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   効果文は見つかったのに挿入できなかったカードがある場合は（全体の挿入
   件数が0件でなくても）「挿入失敗: 原文 / level / Show Effectsの有無 /
   実際に選んだ挿入先要素のタグ名とクラス名 / 例外の有無」を必ず表示する。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   挿入位置は 1) 「Show Effects」リンクがあればその直後（リンクの内側に
   紛れ込まないよう `closest('a, button')` でクリック領域まで遡ってから
   挿入する） 2) 無ければ効果文の最後のテキスト要素の直後 3) それも
   見つからなければ最終手段としてカード名要素の直後、の優先順位で決める。
   カード名要素は候補収集の時点で必ず見つかっているため、この最終手段だけは
   絶対に失敗しない。`position` や `width` は指定せず周囲の要素にそのまま
   従わせ、背景は付けず、文字色を白、上に細い区切り線を1本入れ、文字
   サイズは英語の効果文と同じ実測値に合わせる。挿入のたびに、box の中に
   既存の効果文ブロックがあれば `querySelectorAll` で全て探して削除して
   から作り直すため、SPA側の再描画でboxのdata属性だけが失われるようなケース
   でも複数のブロックが並ぶことはない。挿入自体が例外を投げた場合は診断用に
   記録し、PROCESSEDには加えず次回の再描画時にまた挑戦する。英語の効果文は
   どこにあっても消さずそのまま残す。
