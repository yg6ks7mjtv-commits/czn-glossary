...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   - 名前特定スキップ: カード名を特定できず、無理に近い候補を採用せず
     ...
     最大5件表示する
   効果文を1件以上書き換えられたときは、
   「原文 / level / 集めたテキストノードの数 / 日本語を入れたノードの
   親要素のタグ名 / 書き換え前の連結英文の先頭30文字」を最大10件表示する。
   カード名の要素などを誤って書き換えていないか、その場で確認できる。
...
4. カード名は glossary が分かっていれば、効果文の有無とは無関係に常に
   ...
   効果文が見つかれば、その探索範囲(scope)内のテキストを書き換える。
   **効果文の探索範囲は名前照合用の（狭い）カード枠 box とは別に扱う**:
   box から親を1階層ずつたどり、その要素の `textContent` が box の
   `textContent` より50文字以上多くなった時点で、そこを探索範囲として
   採用する（＝効果文相当のまとまった量のテキストが新たに含まれるように
   なったとみなす。box がカード名＋種別表示だけのヘッダー部分で、効果文が
   全く別の場所にあるケースに対応するため。固定の階層数だけで判断すると
   届かないことがあった）。最大 `maxEffectSearchClimb`（`selectors.js`、
   既定10）階層まで許可。ただし2つ目の種別表示を含む階層に達したら、その
   1つ手前で打ち切る（3.の歯止め(a)と同じ考え方）。
   ...
   **英語の効果文は1つの要素にまとまっておらず、複数のテキストノードに
   ...
   二重処理を防ぐ（既に付いていればそのスキャンでは何もしない）。対象の
   テキストノードが1つも見つからないカードや、日本語の効果文が無いカード
   には何もしない（英語のまま残る）。
