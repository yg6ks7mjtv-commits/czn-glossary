...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / ◯件の効果文を挿入」という
   トーストが出る。何も処理できなかった場合は「CZN: 対象が見つかりません」と出る
   （マーカー検出やセレクタが実際のページに合っていない可能性が高い）
...
処理の流れ: ページ上で「Show Effects」を含む説明ブロックを検出 → その近くに
ある見出し要素からカード名(英語)を取得 → `glossary.json` で日本語名に変換 →
`effects-ja.json` からその日本語名で効果文を検索 → 見つかれば、英語の説明文を
消さずにその下へ日本語の効果文を追記する。効果文が無いカードは何もしない。
カード名以外のテキストの用語置換は、ブックマークレットと同じアルゴリズム・
同じ辞書（`glossary.json`）で行う。
...
- 既定の検出方式（`extension/selectors.js` の `useMarkerStrategy: true`）は
  「Show Effects」というテキストを含む要素を探し、そこから親を数階層たどって
  説明ブロックを特定、ブロックの近くにある見出し要素（h1〜h6等）をカード名として
  読む、という2段構え
- マーカーの文字列・たどる階層数・見出し候補のセレクタは
  すべて `extension/selectors.js` に切り出してある。実際のページで devtools を
...
一発では動かない前提で作ってあるので、画面右下のトースト表示と `CZN_DEBUG`
を見ながら `selectors.js` を育てていくこと。
