...
5. 動作すると画面右下に3秒だけ「CZN: ◯件を置換 / カード枠◯件 / 名前取得◯件 /
   ...
   表示中か非表示か / 親要素のoverflow / display・visibility」の内訳を
   最大10件表示する。効果文データの件数と挿入件数が一致しない（重複挿入や
   level違いへの誤挿入が疑われる）ときや、挿入したブロックが非表示になって
   しまうときに、その場で原因を切り分けられる。
...
4. `effects-ja.json` をそのカード名（日本語）・キャラ・levelの組で検索する。
   ...
   効果文が見つかれば、カード枠（box）の「中」ではなく box の直後の兄弟
   要素として、独立したブロックで日本語の効果文を追加する（box の中は
   画像などが `position:absolute` で配置されていることがあり、中に入れると
   重なってしまうため）。box の親要素には `overflow:visible` を明示的に
   付与してはみ出しを防ぎ、幅は親からの継承やパーセント指定に頼らず、
   box の実測幅（`getBoundingClientRect()`）をpxで直接指定する。
   `position:static`・`display:block`・`white-space:normal`・
   `word-break:normal`、文字サイズはカード名見出しの実測値に合わせ、
   背景は濃い半透明の黒、文字は白で表示する。挿入のたびに、box の親要素の
   中に既存の効果文ブロックがあれば `querySelector` で探して削除してから
   作り直すため、SPA側の再描画でboxのdata属性だけが失われるようなケースでも
   同じカードに複数のブロックが並ぶことはない。英語の効果文がどこにあっても
